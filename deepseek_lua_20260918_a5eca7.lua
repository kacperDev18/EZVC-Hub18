-- EZVC Hub v2.6 - Fixed clipboard detection (direct global refs)
...
lb(hdr,"v2.6",92,0,90,38,MT,9,false)
...
-- ============================================================
-- SHARE MACRO PAGE (fixed clipboard, single Copy button)
-- ============================================================
-- Clipboard resolution using DIRECT global refs (respects __index metamethods).
-- This is the key fix: previous code used rawget(getfenv(), name) which bypasses
-- the executor's global injection and always returned nil.
local CLIP_FN, CLIP_NAME
local function resolveClipboard()
    if CLIP_FN then return CLIP_FN, CLIP_NAME end
    -- Direct global access — works on Delta, Xeno, Synapse, Arceus, etc.
...
ac(copyBtn.MouseButton1Click:Connect(function()
    print("[EZVC][Share] Copy clicked. sel='"..tostring(sel).."'")
    ...
    if #data==0 then
        print("[EZVC][Share] Macro has 0 actions")
        N("EZVC","Macro is empty or invalid")
        return
    end
    ...
    if type(json)~="string" or #json<5 then
        print("[EZVC][Share] JSON empty")
        N("EZVC","Exported macro is empty.")
        return
    end
    print("[EZVC][Share] JSON len="..#json)
    local fn,name=resolveClipboard()
    if not fn then
        print("[EZVC][Share] No clipboard fn found. Available globals checked: setclipboard, toclipboard, writeclipboard, set_clipboard, getgenv.*, syn.*, Clipboard.set")
        N("EZVC","Clipboard function is not available in this runtime.")
        return
    end
    print("[EZVC][Share] Using clipboard: "..name)
    local cok,cerr=pcall(fn,json)
    if not cok then
        print("[EZVC][Share] Clipboard call error: "..tostring(cerr))
        N("EZVC","Clipboard error: "..tostring(cerr))
        return
    end
    print("[EZVC][Share] Clipboard call returned OK")
...
rfC()
N("EZVC","Ready v2.6")
print("[EZVC] v2.6 loaded | mode="..(HOOK_OK and HOOK_MODE or "passive").." | clipboard="..tostring(CLIP_NAME or "none"))