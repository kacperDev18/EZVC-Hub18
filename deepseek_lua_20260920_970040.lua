-- EZVC Hub v2.8 - Proper shutdown + single-shot playback
...
-- Runtime state flags (declared early so all loops see them)
...
-- Play Macro MUST always start OFF. Config is never source of truth for playback.
Cfg.pl=false
sel=Cfg.sel or ""
local _init=true
local _pend=false
...
lb(hdr,"v2.8",92,0,90,38,MT,9,false)
...
local function ask()
    ...
    ac(yb.MouseButton1Click:Connect(function()
        if shuttingDown then return end
        -- Set shutdown flag FIRST so all loops terminate immediately
        shuttingDown=true
        playbackActive=false
        playbackSession=playbackSession+1
        rec.active=false
...
-- ============================================================
-- PLAYBACK — single worker, session-token guarded
-- ============================================================
...
local function startPlayback()
    ...
    task.spawn(function()
        ...
        -- Exit path: natural end, external stop, or shutdown
        if playbackSession==mySession then
            -- Natural end (not cancelled by external stopPlayback)
            playbackActive=false
            playbackSession=playbackSession+1
            setStatus("Idle")
...
task.spawn(function()
    ...
    -- Play Macro ALWAYS starts off, regardless of previous session
    pToggle.SetValue(false)
    if CLIP_FN then
...
rfC()
N("EZVC","Ready v2.8")
print("[EZVC] v2.8 loaded | mode="..(HOOK_OK and HOOK_MODE or "passive").." | clipboard="..tostring(CLIP_NAME or "none"))