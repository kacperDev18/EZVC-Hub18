-- EZVC Hub v2.5 - Fixed clipboard with full debug tracing
...
lb(hdr,"v2.5",92,0,90,38,MT,9,false)
...
-- ============================================================
-- SHARE MACRO PAGE (fixed clipboard w/ full debug)
-- ============================================================
local exportedStr=""
local function getClipboardFn()
    local cands={
        {name="setclipboard",   fn=rawget(getfenv(),"setclipboard")},
        {name="toclipboard",    fn=rawget(getfenv(),"toclipboard")},
        {name="writeclipboard", fn=rawget(getfenv(),"writeclipboard")},
        {name="set_clipboard",  fn=rawget(getfenv(),"set_clipboard")},
    }
    for _,c in ipairs(cands) do
        if type(c.fn)=="function" then return c.fn,c.name end
    end
    local synT=rawget(getfenv(),"syn")
    if type(synT)=="table" then
        if type(synT.setclipboard)=="function" then return synT.setclipboard,"syn.setclipboard" end
        if type(synT.write_clipboard)=="function" then return synT.write_clipboard,"syn.write_clipboard" end
        if type(synT.clipboard)=="table" and type(synT.clipboard.set)=="function" then
            return function(t) synT.clipboard.set(t) end,"syn.clipboard.set"
        end
    end
    return nil
end
local function tryGetClipboard()
    local fn=rawget(getfenv(),"getclipboard")
    if type(fn)=="function" then return fn,"getclipboard" end
    local synT=rawget(getfenv(),"syn")
    if type(synT)=="table" and type(synT.getclipboard)=="function" then return synT.getclipboard,"syn.getclipboard" end
    return nil
end
local sP=pg("Share")
local sc1=cd(sP,"Export",12)
sc1.Size=UDim2.new(1,-24,0,160)
...
local expBtn=btn(sc1,"Export Macro",12,80,240,28,"p")
local copyBtn=btn(sc1,"Copy Macro to Clipboard",12,116,240,28)
ac(expBtn.MouseButton1Click:Connect(function()
    print("[EZVC][Share] Export clicked. sel='"..tostring(sel).."'")
    if sel=="" then N("EZVC","Select a macro first") return end
    local data=loadMacro(sel)
    if not data then
        print("[EZVC][Share] loadMacro returned nil")
        N("EZVC","Macro is empty or invalid")
        return
    end
    print("[EZVC][Share] Loaded "..#data.." actions")
    if #data==0 then N("EZVC","Macro is empty or invalid") return end
    local payload={Name=sel,Actions=data}
    local ok,json=pcall(function() return HS:JSONEncode(payload) end)
    if not ok then
        print("[EZVC][Share] JSONEncode error: "..tostring(json))
        N("EZVC","Failed to generate macro JSON.")
        return
    end
    if type(json)~="string" or #json==0 then
        print("[EZVC][Share] JSON empty after encode")
        N("EZVC","Exported macro is empty.")
        return
    end
    exportedStr=json
    print("[EZVC][Share] exportedStr len="..#exportedStr)
    print("[EZVC][Share] preview: "..exportedStr:sub(1,120))
    N("EZVC","Macro exported successfully.")
end))
ac(copyBtn.MouseButton1Click:Connect(function()
    print("[EZVC][Share] Copy clicked. exportedStr len="..tostring(#exportedStr))
    if type(exportedStr)~="string" or #exportedStr<5 then
        print("[EZVC][Share] exportedStr is empty/nil")
        N("EZVC","Exported macro is empty.")
        return
    end
    local fn,name=getClipboardFn()
    if not fn then
        print("[EZVC][Share] No clipboard fn found")
        N("EZVC","Clipboard function is not available in this runtime.")
        return
    end
    print("[EZVC][Share] Using clipboard fn: "..name.." len="..#exportedStr)
    local ok,err=pcall(fn,exportedStr)
    if not ok then
        print("[EZVC][Share] Clipboard error: "..tostring(err))
        N("EZVC","Clipboard error: "..tostring(err))
        return
    end
    local gfn=gname
    local gFnName
    gfn,gFnName=tryGetClipboard()
    if gfn then
        local ok2,back=pcall(gfn)
        if ok2 and type(back)=="string" and back==exportedStr then
            print("[EZVC][Share] Verify OK via "..gFnName.." len="..#back)
            N("EZVC","Macro JSON copied to clipboard.")
            return
        elseif ok2 then
            print("[EZVC][Share] Verify mismatch. back len="..tostring(type(back)=="string" and #back or "nil"))
        end
    end
    N("EZVC","Macro JSON copied to clipboard.")
end))
local sc2=cd(sP,"Import",184)
...
ac(importBtn.MouseButton1Click:Connect(function()
    ...
    print("[EZVC][Share] Import clicked. len="..#raw)
    ...
    if not ok or type(decoded)~="table" then
        print("[EZVC][Share] JSONDecode failed: "..tostring(decoded))
        pendingImport=nil
    ...
    pendingImport=clean
    print("[EZVC][Share] Parsed "..#clean.." actions, name="..tostring(importedName))
...
ac(saveImportBtn.MouseButton1Click:Connect(function()
    ...
    if wj(FO..name..".json",pendingImport) then
        ...
        importBox.Text=""
        importName.Text=""
        print("[EZVC][Share] Saved imported macro: "..name)
        N("EZVC","Saved: "..name)
    else
        N("EZVC","Save failed")
    end
...
task.spawn(function()
    ...
    -- Log which clipboard functions are available
    local fn,name=getClipboardFn()
    if fn then
        print("[EZVC] Clipboard available: "..name)
    else
        print("[EZVC] WARNING: No clipboard function found in this runtime")
    end
end)
go("Main")
rfC()
N("EZVC","Ready v2.5")
print("[EZVC] v2.5 loaded | mode="..(HOOK_OK and HOOK_MODE or "passive"))