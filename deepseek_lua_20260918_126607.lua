-- EZVC Hub v2.4 - Fixed Share Macro system (JSON {Name,Actions})
...
lb(hdr,"v2.4",92,0,90,38,MT,9,false)
...
local function copyToClipboard(txt)
    ...
    if not fn then return false,"Clipboard function is not available." end
    local ok=pcall(fn,txt)
    if not ok then return false,"Clipboard copy failed." end
    return true
end
...
ac(copyBtn.MouseButton1Click:Connect(function()
    ...
    local ok,err=copyToClipboard(exportedStr)
    if ok then N("EZVC","Copied to clipboard.")
    else N("EZVC",err or "Clipboard copy failed.") end
end))
...
rfC()
N("EZVC","Ready v2.4")
print("[EZVC] v2.4 loaded | mode="..(HOOK_OK and HOOK_MODE or "passive"))