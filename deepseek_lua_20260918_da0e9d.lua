-- EZVC Hub v2.3 - Share Macro + Lobby removal + silent recording
...
lb(hdr,"v2.3",92,0,90,38,MT,9,false)
...
ac(expBtn.MouseButton1Click:Connect(function()
    ...
    if not ok then N("EZVC","Encode failed") return end
    exportedStr="EZVC:"..json
    N("EZVC","Exported "..#data.." actions")
end))
ac(copyBtn.MouseButton1Click:Connect(function()
    if exportedStr=="" then N("EZVC","Click Export first") return end
    local fn=setclipboard or toclipboard
    if type(fn)~="function" then N("EZVC","Clipboard not supported") return end
    local ok=pcall(fn,exportedStr)
    if ok then N("EZVC","Copied to clipboard")
    else N("EZVC","Clipboard failed") end
end))
local sc2=cd(sP,"Import",184)
sc2.Size=UDim2.new(1,-24,0,244)
lb(sc2,"Paste macro code",12,26,200,14,SB,10,false)
local importBox=mk("TextBox",{Size=UDim2.new(1,-24,0,60),Position=UDim2.new(0,12,0,44),BackgroundColor3=BG,BorderSizePixel=0,Font=Enum.Font.Gotham,TextSize=10,TextColor3=TX,Text="",PlaceholderText="Paste EZVC:... or raw JSON here",PlaceholderColor3=MT,ClearTextOnFocus=false,TextWrapped=true,MultiLine=true,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top},sc2)
cr(importBox,5)
lb(sc2,"Macro name",12,112,200,14,SB,10,false)
local importName=mk("TextBox",{Size=UDim2.new(1,-24,0,26),Position=UDim2.new(0,12,0,128),BackgroundColor3=BG,BorderSizePixel=0,Font=Enum.Font.Gotham,TextSize=11,TextColor3=TX,Text="",PlaceholderText="e.g. SharedFarm",PlaceholderColor3=MT,ClearTextOnFocus=false},sc2)
cr(importName,5)
...
ac(importBtn.MouseButton1Click:Connect(function()
    ...
    if #raw<5 then N("EZVC","Paste macro code first") return end
    local json=raw
    if json:sub(1,5)=="EZVC:" then json=json:sub(6) end
    json=json:match("^%s*(.-)%s*$") or json
    local ok,decoded=pcall(function() return HS:JSONDecode(json) end)
    ...
    local clean={}
    for _,a in ipairs(decoded) do
        local s=sanitizeAction(a)
        if s then clean[#clean+1]=s end
    end
    if #clean==0 then N("EZVC","No valid actions found") pendingImport=nil return end
    pendingImport=clean
...
rfC()
N("EZVC","Ready v2.3")
print("[EZVC] v2.3 loaded | mode="..(HOOK_OK and HOOK_MODE or "passive"))